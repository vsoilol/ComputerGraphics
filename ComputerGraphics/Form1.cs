...
namespace ComputerGraphics
{
    public partial class Form1 : Form
    {
        ...
        public void AlgorithmSimple(IEnumerable<Edge> edges, int numberColor)
        {
            ...
            for (int y = minEdgeY.StartingPoint.Y; y <= maxEdgeY.EndPoint.Y; y++)
            {
                ...
                for (int i = 0; i < currentPoints.Count / 2; i++)
                {
                    for (int x = currentPoints[i + i].X; x <= currentPoints[i + 1 + i].X; x++)
                    {
                        ...
                        bitmap.SetPixel(x, y, SetCells.Bitmaps[numberColor].GetPixel(xk, yk));
        ...
        private void button1_Click(object sender, EventArgs e)
        {
            ...
            {
                AlgorithmSimple(edges, (int)numberColor.Value);
            }

            if (gradient.Checked)
            {
                AlgorithmGradient(edges);
            }
        }


        public void AlgorithmGradient(IEnumerable<Edge> edges)
        {
            Random random = new Random();
            ...
            int numberRowsWithColor = maxEdgeY.EndPoint.Y / 26;
            ...
            for (int y = minEdgeY.StartingPoint.Y; y <= maxEdgeY.EndPoint.Y; y++)
            {
                ...
                for (int i = 0; i < currentPoints.Count / 2; i++)
                {
                    for (int x = currentPoints[i + i].X; x <= currentPoints[i + 1 + i].X; x++)
                    {
                        ...
                        bitmap.SetPixel(x, y, SetCells.Bitmaps[nextCells].GetPixel(xk, yk));
                        pictureBox1.Image = bitmap;
                    }
                }
                ...
                if (numberRowsWithColor == 0 && nextCells < 25)
                {
                    numberRowsWithColor = maxEdgeY.EndPoint.Y / 26;
                    nextCells++;
                }
            }
        }
